...
class FM9ParameterAuditor:
    ...
    def audit_parameters(self, data: Dict[str, Any]) -> None:
        ...
        self.audit_results["total_parameters"] = sum(
            len(params) for params in all_parameters.values()
        )
    ...
    def _audit_block_types(
        self, block_types: Set[str], config_data: Dict, reference_data: Dict
    ) -> None:
    ...
    def _audit_parameter_sourcing(
        self, all_parameters: Dict[str, Set[str]], reference_data: Dict
    ) -> None:
    ...
    def _audit_parameter_ranges(
        self, blocks_data: List[Dict], reference_data: Dict
    ) -> None:
    ...
    def generate_report(self, output_file: Path = None) -> str:
        ...
        report_lines.append(
            f"- Total blocks audited: {self.audit_results['total_blocks']}"
        )
        report_lines.append(
            f"- Total parameters audited: {self.audit_results['total_parameters']}"
        )
        report_lines.append(
            f"- Missing parameters: {len(self.audit_results['missing_parameters'])}"
        )
        report_lines.append(
            f"- Out of range parameters: {len(self.audit_results['out_of_range_parameters'])}"
        )
        report_lines.append(
            f"- Missing block types: {len(self.audit_results['missing_block_types'])}"
        )
        ...
        if self.audit_results["missing_block_types"]:
            ...
            for missing in self.audit_results["missing_block_types"]:
                report_lines.append(
                    f"- {missing['block_type']}: {missing['missing_from']}"
                )
            report_lines.append("")
        ...
        if self.audit_results["missing_parameters"]:
            ...
            for missing in self.audit_results["missing_parameters"]:
                report_lines.append(
                    f"- {missing['block_type']}.{missing['parameter']}: {missing['missing_from']}"
                )
            report_lines.append("")
        ...
        if self.audit_results["out_of_range_parameters"]:
            ...
            for out_of_range in self.audit_results["out_of_range_parameters"]:
                report_lines.append(
                    f"- {out_of_range['block_type']}.{out_of_range['parameter']}: "
        ...
        # Compliance status
        total_issues = (
            len(self.audit_results["missing_parameters"])
        ...
        if total_issues == 0:
            ...
            report_lines.append(
                "All parameters are properly sourced from FM9 reference files."
            )
        else:
            report_lines.append("## ❌ Compliance Status: FAILED")
            report_lines.append(
                f"Found {total_issues} compliance issues that must be resolved."
            )
...
def main():
    ...
    parser.add_argument(
        "--ref", required=True, help="Path to FM9 comprehensive reference JSON file"
    )
