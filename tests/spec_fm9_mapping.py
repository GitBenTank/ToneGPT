...
class TestFM9ParameterMapping:
    ...
    def test_no_invented_parameters(self):
        ...
        for block_type, expected_params in key_parameters.items():
            for param_name in expected_params:
                assert self._parameter_exists_in_reference(
    ...
    def test_gear_mapping_accuracy(self):
        ...
        marshall_blocks = [
            ...
            if block.get("category") == "amp"
            and "brit" in block.get("name", "").lower()
        ]
        ...
        mesa_blocks = [
            ...
            for block in self.blocks_data
            if block.get("category") == "amp"
            and "recto" in block.get("name", "").lower()
        ]
        ...
        fender_blocks = [
            ...
            for block in self.blocks_data
            if block.get("category") == "amp"
            and "fender" in block.get("name", "").lower()
        ]
        assert len(fender_blocks) > 0, "No Fender blocks found"

    def _validate_parameter_range(
        self, param_name: str, param_value: Any, block_type: str
    ):
        ...
        if block_type in self.fm9_reference["fm9_reference"]:
            ...
            if (
                "key_parameters" in block_ref
                and param_name in block_ref["key_parameters"]
            ):
    ...
    def test_feature_flags_for_future_multi_device(self):
        ...
        for py_file in core_dir.glob("*.py"):
            ...
            if "multi_device" in content or "multi-device" in content:
                assert (
                    "FEATURE_MULTI_DEVICE" in content
...
def test_fm9_params_source_of_truth():
    ...
    ref = json.loads(
        (ROOT / "tonegpt" / "data" / "fm9_comprehensive_reference.json").read_text()
    )
...
def test_feature_flag_defined_for_future_multi_device():
    ...
    assert (
        "FEATURE_MULTI_DEVICE" in flags_content
    ), "FEATURE_MULTI_DEVICE flag not defined"
