...
class AmpsListParser:
    ...
    def _determine_category(self, amp_name: str) -> str:
        ...
        if any(
            word in amp_lower
            for word in ["fender", "tweed", "bassman", "twin", "deluxe", "champ"]
        ):
            return "fender"
        elif any(
            word in amp_lower
            for word in ["marshall", "jcm", "plexi", "jtm", "jvm", "brit"]
        ):
            return "marshall"
        elif any(
            word in amp_lower
            for word in ["mesa", "boogie", "mark", "rectifier", "dual", "triple"]
        ):
            return "mesa_boogie"
    ...
    def _extract_model(self, amp_name: str) -> str:
        ...
        model = re.sub(
            r"^(Fender|Marshall|Mesa|Boogie|Vox|Orange|Peavey|Engl|Diezel|Bogner|Soldano)\s*",
            "",
            model,
            flags=re.IGNORECASE,
        )
    ...
    def _extract_description(self, content: str) -> str:
        ...
        for sentence in sentences:
            ...
            if (
                len(sentence) > 20
                and not sentence.startswith("Model:")
                and not sentence.startswith("Cab:")
            ):
                return sentence
