...
class EnhancedAIToneGenerator:
                    ...
                    if isinstance(param_data, dict) and 'min' in param_data:
                        ...
                        for sub_param, sub_data in param_data.items():
                            if isinstance(sub_data, dict) and 'min' in sub_data:
                                flattened[f"{category}_{param_name}_{sub_param}"] = sub_data
                            elif isinstance(sub_data, dict):
                                # Further nested
                                for sub_sub_param, sub_sub_data in sub_data.items():
                                    if isinstance(sub_sub_data, dict) and 'min' in sub_sub_data:
                                        flattened[f"{category}_{param_name}_{sub_param}_{sub_sub_param}"] = sub_sub_data
    ...
    def _load_firmware_blocks(self) -> Dict:
        ...
        try:
            firmware_file = Path(__file__).parent.parent.parent / "data" / "firmware_blocks_integrated.json"
                ...
                for block in firmware_blocks:
                    ...
                    if 'Fractal Audio FM9' in real_world:
                        ...
                        if 'amp' in description.lower():
                            real_world = f"FM9 Amp Model {block.get('name', '').split('_')[-1]}"
                        elif 'cab' in description.lower():
                            real_world = f"FM9 Cab Model {block.get('name', '').split('_')[-1]}"
                    ...
                    organized_firmware[category].append({
                        ...
                        'footswitch_assignable': block.get('footswitch_assignable', True),
    ...
    def _load_amp_models(self) -> List[str]:
        ...
        known_amps = [
            "Brit 800", "Brit 900", "Brit JVM", "Brit Super", "Brit Plexi",
            "USA IIC+", "USA Lead", "USA Lead 2", "USA Bassguy", "USA Clean",
            "FAS Modern", "FAS Brootalz", "FAS Wreck", "FAS Crunch",
            "Euro Blue", "Euro Red", "Euro Uber", "Euro Blue OD",
            "Shiva Clean", "Shiva Lead", "Shiva Clean 2", "Shiva Lead 2"
        ]
    ...
    def _load_cab_models(self) -> List[str]:
        ...
        known_cabs = [
            "4x12 Brit TV", "4x12 Brit 75W", "4x12 Brit 80W", "4x12 Brit 100W",
            "4x12 USA Trad", "4x12 USA Modern", "4x12 USA Lead", "4x12 USA Bass",
            "2x12 Brit 30W", "2x12 Brit 40W", "1x12 Brit 30W", "1x12 Brit 40W",
            "4x10 USA Bass", "1x15 USA Bass", "4x12 Euro 80W", "4x12 Euro 100W"
        ]
    ...
    def _load_artist_mappings(self) -> Dict:
        ...
        return {
            "deftones": {
                ...
                "gear": {
                    "amps": ["MESA BOOGIE MARK IIC+", "MESA BOOGIE DUAL RECTIFIER", "MESA BOOGIE TRIPLE RECTIFIER", "FAS BROOTALZ", "EURO UBER"],
            ...
            "tool": {
                ...
                "gear": {
                    "amps": ["USA LEAD", "BRIT 800", "FAS MODERN", "MESA BOOGIE MARK IIC+", "MARSHALL JCM800"],
    ...
    def _load_guide_data(self) -> Dict:
        ...
        try:
            ...
            guide_file = Path(__file__).parent.parent.parent / "verified_guide_data.json"
            with open(guide_file, 'r') as f:
                guide_data = json.load(f)
            
            # Also load blocks guide data
            blocks_guide_file = Path(__file__).parent.parent.parent / "blocks_guide_data.json"
            ...
            return {
                ...
                "footswitch": {
                    "footswitch_functions": [
                        "Scene switching", "Block bypass", "Channel switching",
                        "Parameter control", "Looper control", "Tap tempo"
                    ]
                }
            }
    ...
    def _load_fm9_reference(self) -> Dict:
        ...
        try:
            reference_file = Path(__file__).parent.parent / "data" / "fm9_comprehensive_reference.json"
        ...
        return {
            ...
            **self.advanced_blocks.get_all_advanced_blocks(),
            ...
            "harmonizer": {
                ...
                "parameters": {
                    "pitch1": {"min": -24, "max": 24, "default": 0, "unit": "semitones"},
                    "pitch2": {"min": -24, "max": 24, "default": 0, "unit": "semitones"},
                    "mix": {"min": 0, "max": 100, "default": 50, "unit": "%"}
                }
            },
            "gain": {
    ...
    def _generate_tone_patch(self, structure: Dict) -> Dict:
        ...
        if structure["genre"] in ["metal", "rock", "funk", "blues"] or random.random() < 0.7:
            patch["dynamics"] = self._generate_dynamics_block(structure)
        
        # Add modulation for most genres (chorus/flanger are common)
        if structure["genre"] in ["ambient", "rock", "funk", "blues"] or random.random() < 0.6:
            patch["modulation"] = self._generate_modulation_block(structure)
        
        # Add pitch effects for lead tones
        if "lead" in structure.get("characteristics", []) or structure["genre"] in ["rock", "metal"] or random.random() < 0.3:
    ...
    def _generate_drive_block(self, position: int, structure: Dict) -> Dict:
        ...
        if drive_blocks:
            drive_types = [block.get('name', '') for block in drive_blocks if block.get('name')]
        
        if not drive_types:
            # Fallback to firmware data
            if "gain" in self.firmware_blocks:
                drive_types = [block.get("real_world", "") for block in self.firmware_blocks["gain"]]
        ...
        if 'tube screamer' in query or 'ts808' in query or 'ts9' in query:
            ts_drives = [drive for drive in drive_types if 'ts' in drive.lower() or 'tube' in drive.lower() or 'screamer' in drive.lower()]
            if ts_drives:
        ...
        if not selected_drive:
            ...
            if genre == 'blues':
                blues_drives = [drive for drive in drive_types if any(word in drive.lower() for word in ['ts', 'tube', 'screamer', 'klon'])]
                selected_drive = random.choice(blues_drives) if blues_drives else random.choice(drive_types)
            elif genre == 'metal':
                metal_drives = [drive for drive in drive_types if any(word in drive.lower() for word in ['rat', 'distortion', 'fuzz', 'boost'])]
                selected_drive = random.choice(metal_drives) if metal_drives else random.choice(drive_types)
            elif genre == 'jazz':
                # Jazz typically uses clean boosts or light overdrive
                jazz_drives = [drive for drive in drive_types if any(word in drive.lower() for word in ['boost', 'clean', 'light'])]
                selected_drive = random.choice(jazz_drives) if jazz_drives else random.choice(drive_types)
            else:
    ...
    def _generate_amp_block(self, structure: Dict) -> Dict:
        ...
        amp_blocks = self.blocks_data.get('amp', [])
        if amp_blocks:
            amp_types = [block.get('name', '') for block in amp_blocks if block.get('name')]
        ...
        if 'marshall' in query or 'jcm' in query or '800' in query:
            marshall_amps = [amp for amp in amp_types if 'brit' in amp.lower() or 'marshall' in amp.lower() or '800' in amp.lower()]
            if marshall_amps:
                selected_amp = random.choice(marshall_amps)
        elif 'mesa' in query or 'boogie' in query:
            mesa_amps = [amp for amp in amp_types if 'mesa' in amp.lower() or 'boogie' in amp.lower() or 'recto' in amp.lower()]
            if mesa_amps:
                selected_amp = random.choice(mesa_amps)
        elif 'fender' in query:
            fender_amps = [amp for amp in amp_types if 'fender' in amp.lower() or 'tweed' in amp.lower() or 'deluxe' in amp.lower()]
            if fender_amps:
        ...
        if not selected_amp:
            if genre == 'metal':
                metal_amps = [amp for amp in amp_types if any(word in amp.lower() for word in ['mesa', 'recto', '5150', 'mark', 'brit', '800'])]
                selected_amp = random.choice(metal_amps) if metal_amps else random.choice(amp_types)
            elif genre == 'blues':
                blues_amps = [amp for amp in amp_types if any(word in amp.lower() for word in ['fender', 'tweed', 'deluxe', 'vox', 'ac30'])]
                selected_amp = random.choice(blues_amps) if blues_amps else random.choice(amp_types)
            elif genre == 'jazz':
                jazz_amps = [amp for amp in amp_types if any(word in amp.lower() for word in ['fender', 'tweed', 'deluxe', 'clean'])]
                selected_amp = random.choice(jazz_amps) if jazz_amps else random.choice(amp_types)
            else:
        ...
        for channel in ["A", "B", "C", "D"]:
            ...
            if structure["genre"] == "metal":
                if any("IIC+" in amp or "Brootalz" in amp or "Uber" in amp for amp in amp_types):
                    metal_amps = [amp for amp in amp_types if any(word in amp for word in ["IIC+", "Brootalz", "Uber"])]
                    if metal_amps:
            ...
            treble_base = random.uniform(profile["treble"]["min"], profile["treble"]["max"])
            
            # Mid often complements the gain setting
            mid_base = profile["mid"]["typical"] + (base_gain - profile["gain"]["typical"]) * 0.2
            ...
            presence_base = max(profile["presence"]["min"], min(profile["presence"]["max"], presence_base))
            
            # Master volume often inversely related to gain for proper gain staging
            master_base = profile["master"]["typical"] - (base_gain - profile["gain"]["typical"]) * 0.3
            master_base = max(profile["master"]["min"], min(profile["master"]["max"], master_base))
    ...
    def _generate_cab_block(self, structure: Dict) -> Dict:
        ...
        for channel in ["A", "B", "C", "D"]:
            ...
            preferred_cabs = [cab for cab in cab_types if any(pref in cab for pref in profile["cab_preferences"])]
            if preferred_cabs:
            ...
            base_low_cut = random.uniform(profile["low_cut"]["min"], profile["low_cut"]["max"])
            base_high_cut = random.uniform(profile["high_cut"]["min"], profile["high_cut"]["max"])
            base_level = random.uniform(profile["level"]["min"], profile["level"]["max"])
    ...
    def _generate_delay_block(self, structure: Dict) -> Dict:
        ...
        if delay_blocks:
            delay_types = [block.get('name', '') for block in delay_blocks if block.get('name')]
        
        if not delay_types:
            delay_types = ["Digital Stereo Delay", "Analog Delay", "Tape Delay", "Multi Delay"]
    ...
    def _generate_reverb_block(self, structure: Dict) -> Dict:
        ...
        if reverb_blocks:
            reverb_types = [block.get('name', '') for block in reverb_blocks if block.get('name')]
        
        if not reverb_types:
            reverb_types = ["Room Reverb", "Hall Reverb", "Plate Reverb", "Spring Reverb"]
        
        # Genre-specific reverb profiles
        genre_reverb_profiles = {
            "metal": {
                "type": random.choice([r for r in reverb_types if "Plate" in r or "Room" in r]) if reverb_types else "Medium Plate",
        ...
        for channel in ["A", "B", "C", "D"]:
            ...
            base_decay = random.uniform(profile["decay"]["min"], profile["decay"]["max"])
            base_room_size = random.uniform(profile["room_size"]["min"], profile["room_size"]["max"])
    ...
    def _generate_modulation_block(self, structure: Dict) -> Dict:
        ...
        if modulation_blocks:
            modulation_types = [block.get('name', '') for block in modulation_blocks if block.get('name')]
    ...
    def _generate_dynamics_block(self, structure: Dict) -> Dict:
        ...
        for channel in ["A", "B", "C", "D"]:
            # Base parameters from genre profile
            base_threshold = random.uniform(profile["threshold"]["min"], profile["threshold"]["max"])
            base_ratio = random.uniform(profile["ratio"]["min"], profile["ratio"]["max"])
            base_attack = random.uniform(profile["attack"]["min"], profile["attack"]["max"])
            base_release = random.uniform(profile["release"]["min"], profile["release"]["max"])
            base_level = random.uniform(profile["level"]["min"], profile["level"]["max"])
        ...
        try:
            ...
            for block_name, block_data in tone_patch.items():
                if block_data.get("enabled", False) and "parameters" in block_data:
                    self._validate_block_parameters(block_name, block_data["parameters"])
    ...
    def _validate_block_parameters(self, block_name: str, parameters: Dict) -> None:
        ...
        for param_name, param_value in parameters.items():
            if isinstance(param_value, (int, float)):
                ...
                if param_name in ["gain", "bass", "mid", "treble", "presence", "master"]:
                    if not (0.0 <= param_value <= 10.0):
                        print(f"Warning: {block_name}.{param_name} = {param_value} is outside normal range (0-10)")
                elif param_name in ["level"]:
                    if not (-20.0 <= param_value <= 20.0):
                        print(f"Warning: {block_name}.{param_name} = {param_value} is outside normal range (-20 to 20)")
                elif param_name in ["mix"]:
                    if not (0.0 <= param_value <= 100.0):
                        print(f"Warning: {block_name}.{param_name} = {param_value} is outside normal range (0-100)")
    ...
    def analyze_tone_complexity(self, tone_patch: Dict) -> Dict:
        ...
        complexity_factors = {
            "active_blocks": analysis["active_blocks"] * 10,  # 10 points per active block
            "parameter_count": min(analysis["parameter_count"] * 2, 30),  # 2 points per parameter, max 30
            "effect_blocks": len([bt for bt in analysis["block_types"] if bt in ["delay", "reverb", "modulation", "pitch"]]) * 15,  # 15 points per effect
            "drive_blocks": len([bt for bt in analysis["block_types"] if "drive" in bt]) * 20  # 20 points per drive
        }
        ...
        analysis["technical_analysis"] = {
            ...
            "effect_density": len([bt for bt in analysis["block_types"] if bt in ["delay", "reverb", "modulation", "pitch"]]) / max(analysis["active_blocks"], 1),
            "parameter_density": analysis["parameter_count"] / max(analysis["active_blocks"], 1),
            "complexity_level": "simple" if analysis["complexity_score"] < 30 else "moderate" if analysis["complexity_score"] < 70 else "complex"
        }
    ...
    def generate_comprehensive_tone(self, query: str, tone_name: str = None) -> Dict[str, any]:
        """
        ...
        if not tone_name:
            tone_name = f"ToneGPT_{query.replace(' ', '_')}_{random.randint(1000, 9999)}"
        ...
        comprehensive_summary = self.comprehensive_system.get_comprehensive_tone_summary(tone_id)
