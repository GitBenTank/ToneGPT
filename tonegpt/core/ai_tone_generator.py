 ...
 random
            ...
            with open(amps_file, "r") as f:
                ...
                if isinstance(data, list):
                    ...
                    if data and isinstance(data[0], str):
                        ...
                        # Dictionary list with 'name' key (our format)
                        return [
                            item.get("name", "")
                            for item in data
    ...
    def _generate_tone_patch(self, structure: Dict) -> Dict:
        ...
        if structure["drive_blocks"] >= 1:
            patch["drive_1"] = self._generate_drive_block(structure)
        if structure["drive_blocks"] >= 2:
            patch["drive_2"] = self._generate_drive_block(structure)
    ...
    def _generate_drive_block(self, structure: Dict) -> Dict:
        ...
        if not drive_types:
            drive_types = DEFAULT_DRIVE_TYPES
        ...
        for channel in ["A", "B", "C", "D"]:
            ...
            if structure["genre"] == "metal":
                if drive_types:
                    ...
                    metal_drives = [
                        d
                        for d in DEFAULT_DRIVE_TYPES
                        if any(
                            word in d.lower()
                            for word in ["distortion", "fuzz", "boost"]
                        )
                    ]
                    drive_type = random.choice(metal_drives)
        ...
        for channel in ["A", "B", "C", "D"]:
            ...
            if structure["genre"] == "metal":
                metal_amps = [
                    ...
                    if any(
                        word in amp.lower()
                        for word in ["modern", "lead", "brown", "mesa"]
                    )
                ]
                if metal_amps:
    ...
    def _generate_cab_block(self, _structure: Dict) -> Dict:
    ...
    def _generate_eq_block(self, structure: Dict) -> Dict:
        ...
        return {"enabled": True, "type": eq_type, "parameters": params}
    ...
    def _generate_reverb_block(self, structure: Dict) -> Dict:
        ...
        if "reverb" in self.blocks_data:
            reverb_types = [
                block.get("name", "") for block in self.blocks_data["reverb"]
            ]
